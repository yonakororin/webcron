...
# catch_alert: コマンドを実行し、失敗時にMattermostへアラートを送信するラッパースクリプト
...
send_alert() {
    ...
    # Mattermost attachment の text (stderrをコードブロックで囲む)
    local lf=$'\n'
    local stderr_text=""
    stderr_text+="**スタックトレース (標準エラー出力):**${lf}"
    stderr_text+="\`\`\`${lf}${stderr_output}${lf}\`\`\`"

    # Mattermost向けのattachmentペイロード作成 (Slack互換フォーマット)
    # color: 赤 (#FF0000)
    local payload
    payload=$(jq -nc \
    ...
    if [ $? -eq 0 ]; then
        echo "Mattermostへアラートを送信しました。" >&2
    else
        echo "Mattermostへのアラート送信に失敗しました。" >&2
    fi
}
